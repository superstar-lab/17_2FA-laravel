...
@section('content')
    ...
    <div class="card card-custom overflow-hidden">
        <div class="card-body p-0">
            ...
            <div class="row justify-content-center bgi-size-cover bgi-no-repeat py-8 px-8 py-md-27 px-md-0" style="padding: 50px 10px;background-image: url('{{asset('dashboard/media/bg/bg-6.jpg')}}');">
                <div class="col-md-9">
                    <div class="d-flex justify-content-between pb-10 pb-md-20 flex-column flex-md-row">
                        <h1 class="display-4 text-white font-weight-boldest mb-10">Recharge</h1>
                    ...
                    <div class="d-flex justify-content-between text-white pt-6">
                        <div class="d-flex flex-column flex-root">
                            ...
                            <span class="opacity-70">{{$recharge->created_at->toDateString()}}</span>
                        </div>
                        <div class="d-flex flex-column flex-root">
                            <span class="font-weight-bolder mb-2">TRANSACTION ID</span>
                            <span class="opacity-70">{{$recharge->txid}}</span>
                        </div>
                        <div class="d-flex flex-column flex-root">
                            <span class="font-weight-bolde mb-2r">UPDATED AT</span>
                            <span class="opacity-70">{{$recharge->updated_at->toDateString()}}</span>
                        </div>
                        <div class="d-flex flex-column flex-root">
                            <span class="font-weight-bolder mb-2">INVOICE TO.</span>
                            <span class="opacity-70">{{$recharge->user->name}}<br> {{$recharge->user->email}}</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="d-flex justify-content-between flex-column flex-md-row font-size-lg">
                        <div class="d-flex flex-column mb-10 mb-md-0">
                            ...
                            <div class="d-flex justify-content-between mb-3">
                                <span class="mr-15 font-weight-bold">Operator Name:</span>
                                <span class="text-right">{{$recharge->operator->name ?? 'Removed'}}</span>
                            </div>
                            <div class="d-flex justify-content-between mb-3">
                                <span class="mr-15 font-weight-bold">Phone Number:</span>
                                <span class="text-right">{{$recharge->phone}}</span>
                            </div>
                            <div class="d-flex justify-content-between mb-3">
                                <span class="mr-15 font-weight-bold">Amount in BTC</span>
                                <span class="text-right">{{$recharge->amount_btc}}</span>
                            </div>
                            <div class="d-flex justify-content-between mb-3">
                                <span class="mr-15 font-weight-bold">Amount in USD</span>
                                <span class="text-right">{{$recharge->amount_usd}}</span>
                            </div>
                            <div class="d-flex justify-content-between mb-3">
                                <span class="mr-15 font-weight-bold">Status</span>
                                <span class="text-right">{{$recharge->status}}</span>
                            </div>
                            <div class="d-flex justify-content-between mb-3">
                                <span class="mr-15 font-weight-bold">Extra Information</span>
                                <span class="text-right">{{$recharge->note}}</span>
                            </div>
                        </div>
                        <div class="d-flex flex-column text-md-right">
                            <span class="font-size-lg font-weight-bolder mb-1">RECHARGE AMOUNT</span>
                            <span class="font-size-h2 font-weight-boldest text-danger mb-1">N {{$recharge->amount_naira}}</span>
                        </div>
                    </div>
                    <div class="d-flex justify-content-between mb-3">
                        <span class="mr-15 font-weight-bold">Comment</span>
                        <span class="text-right">{{$recharge->comment}}</span>
                    </div>
                </div>
            </div>
            <!-- end: Invoice footer-->
